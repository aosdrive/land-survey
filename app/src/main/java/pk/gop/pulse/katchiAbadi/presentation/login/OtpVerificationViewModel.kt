...
class OtpVerificationViewModel @Inject constructor(
    ...
    private val _otpVerification = MutableStateFlow<Resource<Unit>>(Resource.Unspecified())
    val otpVerification = _otpVerification.asStateFlow()
    ...
    fun otpVerification(cnic: String, otp: Int) {
        viewModelScope.launch {
            _otpVerification.value = Resource.Loading()
            when (val validationResult = validationUseCase.otpVerificationUseCase(cnic, otp)) {
                is Resource.Success -> {
                        _otpVerification.value = Resource.Success(Unit)
                }

                is Resource.Error -> {
                    _otpVerification.value = Resource.Error(validationResult.message.toString())
                }
